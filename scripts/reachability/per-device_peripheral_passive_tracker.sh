#!/bin/bash
PV="proverif"
MODLE="../../model/reachability/Per-device_peripheral_passive.pv"
...
prepare "Unpaired-41-P1"
echo "14. Verify whether an UNPAIRED passive tracker can track the BLE-only vicitm device using IRK"
m4 -DBLEUNPAIREDIRK -DFIXSCAN $MODLE > $TMP
analyze

prepare "Unpaired-42-P1"
echo "15. Verify whether an UNPAIRED passive tracker can track the BLE-only vicitm device using LTK"
m4 -DBLEUNPAIREDLTK $MODLE > $TMP
analyze

prepare "Unpaired-43-P1"
echo "16. Verify whether an UNPAIRED passive tracker can track the BLE-only vicitm device using CSRK"
m4 -DBLEUNPAIREDCSRK $MODLE > $TMP
analyze

prepare "Unpaired-44-P1"
echo "17. Verify whether an UNPAIRED passive tracker can track the BC/BLE vicitm device using the identity address (ID_ADDR)"
