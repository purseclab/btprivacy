#!/bin/bash
PV="proverif"
MODLE="../../model/observational_equivalence/Per-device_central_active.pv"
...
prepare "Never-2-IRK"
echo "2. Verify whether a NEVER paired passive tracker can track the BLE-only vicitm device using IRK (false attack)"
...
echo "8. Verify whether a PAIRED passive tracker can track the BLE-only vicitm device using LTK (cannot prove)"
...
echo "13. Verify whether an UNPAIRED passive attacker can track the BLE-only vicitm device using LTK (cannot prove)"
